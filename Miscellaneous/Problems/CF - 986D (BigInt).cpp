...
#define polar(r,a)  (cplx<long double>){r*cos(a),r*sin(a)}

const int DIG = 9, FDIG = 4;
const int BASE = 1e9, FBASE = 1e4;
typedef cplx<long double> Cplx;
...
struct BigInt {
  ...
  static vector<Cplx> FFT(vector<Cplx> v, int dir = 1) {
    ...
    for (int k = 0, a = 0; k < n; ++k, a = rev_incr(a, n))
      V[a] = v[k] / (long double)(dir > 0 ? 1 : n);
    for (int m = 2; m <= n; m <<= 1) {
      wm = polar( (long double)1, dir * 2 * PI / m );
      for (int k = 0; k < n; k += m) {
        w = 1;
  ...
  static void convolution(const vector<int>& a, const vector<int>& b, vector<int>& c) {
    ...
    long long carry = 0;
    for (int i = 0; i < n; i++) {
      long long t = (long long)(cv[i].real() + 0.5) + carry;
      carry = t / FBASE;
      c[i] = t % FBASE;
    }
  }
...
BigInt power(BigInt a, long long k) {
  BigInt ans = 1;
  while (k > 0) {
    if (k & 1) ans *= a;
    a *= a;
...
int main() {
    ...
    for (int test = 1; test <= tt; test++) {
        ...
        cin >> s;
        BigInt n = BigInt(s);
        long long mxpow = ((long long)s.size()) / log10(3);
        BigInt k = power(3, max(0LL, mxpow - 6));
        long long ans = 3 * max(0LL, mxpow - 6);
        while (k < n) {
            if (k * 2 >= n) {
                ans += 2;
                break;
